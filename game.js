...
function draw() {
  ...
  nodes.forEach(node => { const weapon = weapons[node.type]; const pulse = 34 + Math.sin(Date.now() / 160) * 6; ctx.beginPath(); ctx.ellipse(node.x, node.y + 20, 27, 9, 0, 0, Math.PI * 2); ctx.fillStyle = '#0008'; ctx.fill(); ctx.beginPath(); ctx.arc(node.x, node.y, pulse, 0, Math.PI * 2); ctx.fillStyle = `${weapon.color}20`; ctx.fill(); ctx.beginPath(); ctx.moveTo(node.x, node.y - 23); ctx.lineTo(node.x + 22, node.y); ctx.lineTo(node.x, node.y + 23); ctx.lineTo(node.x - 22, node.y); ctx.closePath(); ctx.fillStyle = '#101f25'; ctx.fill(); ctx.strokeStyle = weapon.color; ctx.lineWidth = 3; ctx.stroke(); ctx.beginPath(); ctx.arc(node.x, node.y, 8, 0, Math.PI * 2); ctx.fillStyle = weapon.color; ctx.fill(); ctx.fillStyle = '#edf2e9'; ctx.font = 'bold 11px "DM Mono"'; ctx.textAlign = 'center'; ctx.fillText(weapon.name[0], node.x, node.y + 4); });
  ...
  enemies.forEach(enemy => { const type = enemyTypes[enemy.type]; ctx.beginPath(); ctx.ellipse(enemy.x, enemy.y + enemy.r + 7, enemy.r * 1.15, enemy.r * .35, 0, 0, Math.PI * 2); ctx.fillStyle = '#0009'; ctx.fill(); ctx.save(); ctx.translate(enemy.x, enemy.y); ctx.rotate(Math.atan2(player.y - enemy.y, player.x - enemy.x)); ctx.beginPath(); ctx.moveTo(enemy.r + 7, 0); ctx.lineTo(enemy.r * .45, enemy.r * .8); ctx.lineTo(-enemy.r * .8, enemy.r * .65); ctx.lineTo(-enemy.r, 0); ctx.lineTo(-enemy.r * .8, -enemy.r * .65); ctx.lineTo(enemy.r * .45, -enemy.r * .8); ctx.closePath(); ctx.fillStyle = '#0b111d'; ctx.fill(); ctx.strokeStyle = type.color; ctx.lineWidth = enemy.type === 'regent' ? 4 : 3; ctx.stroke(); ctx.fillStyle = type.color; ctx.fillRect(enemy.r * .2, -5, 5, 5); ctx.fillRect(enemy.r * .2, 1, 5, 5); if (enemy.type === 'regent') { ctx.strokeStyle = '#edc968'; ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(-14, -enemy.r * .7); ctx.lineTo(-4, -enemy.r - 10); ctx.lineTo(5, -enemy.r * .7); ctx.lineTo(14, -enemy.r - 10); ctx.stroke(); } ctx.restore(); ctx.fillStyle = '#ed725c'; ctx.fillRect(enemy.x - enemy.r, enemy.y - enemy.r - 9, enemy.r * 2 * Math.max(0, enemy.hp / enemy.maxHp), 4); });
  ...
  if (player) { ctx.globalAlpha = invulnerable > 0 && Math.floor(Date.now() / 80) % 2 ? .35 : 1; ctx.beginPath(); ctx.ellipse(player.x, player.y + 24, 28, 9, 0, 0, Math.PI * 2); ctx.fillStyle = '#000b'; ctx.fill(); ctx.save(); ctx.translate(player.x, player.y); ctx.beginPath(); ctx.moveTo(0, -player.r - 17); ctx.lineTo(player.r + 13, -player.r + 1); ctx.lineTo(player.r + 8, player.r + 17); ctx.lineTo(-player.r - 8, player.r + 17); ctx.lineTo(-player.r - 13, -player.r + 1); ctx.closePath(); ctx.fillStyle = '#9aaca9'; ctx.fill(); ctx.strokeStyle = '#edc968'; ctx.lineWidth = 3; ctx.stroke(); ctx.beginPath(); ctx.moveTo(-player.r - 10, -player.r - 10); ctx.lineTo(0, -player.r - 25); ctx.lineTo(player.r + 10, -player.r - 10); ctx.lineTo(player.r + 4, -player.r - 14); ctx.lineTo(0, -player.r - 20); ctx.lineTo(-player.r - 4, -player.r - 14); ctx.closePath(); ctx.fillStyle = '#edc968'; ctx.fill(); ctx.strokeStyle = '#fff1b0'; ctx.lineWidth = 2; ctx.stroke(); ctx.fillStyle = '#172326'; ctx.fillRect(-6, -3, 4, 8); ctx.fillRect(2, -3, 4, 8); ctx.restore(); ctx.globalAlpha = 1; }
  drawEquippedWeapons();
}
