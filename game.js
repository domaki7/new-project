...
function nodeWeb() {
  ...
  const weaponIds = Object.keys(weapons).filter(id => weapons[id].family === weaponFamily && (equipped.includes(id) || equipped.filter(item => weapons[item]).length < maxWeapons));
  const ids = weaponIds.concat(Object.keys(upgradeTypes).filter(id => upgradeTypes[id].family === weaponFamily));
...
function draw() {
  ...
  nodes.forEach(node => { const item = weapons[node.type] || nodeUpgrades[node.type]; const nearby = player && distance(node, player) < 180; const pulse = (nearby ? 38 : 30) + Math.sin(Date.now() / 160) * (nearby ? 7 : 3); ctx.beginPath(); ctx.ellipse(node.x, node.y + 20, 27, 9, 0, 0, Math.PI * 2); ctx.fillStyle = '#0008'; ctx.fill(); ctx.beginPath(); ctx.arc(node.x, node.y, pulse, 0, Math.PI * 2); ctx.fillStyle = `${item.color}${nearby ? '2c' : '18'}`; ctx.fill(); ctx.beginPath(); ctx.moveTo(node.x, node.y - 23); ctx.lineTo(node.x + 22, node.y); ctx.lineTo(node.x, node.y + 23); ctx.lineTo(node.x - 22, node.y); ctx.closePath(); const relic = ctx.createLinearGradient(node.x - 18, node.y - 18, node.x + 18, node.y + 18); relic.addColorStop(0, '#ffffff55'); relic.addColorStop(.25, item.color); relic.addColorStop(1, '#101f25'); ctx.fillStyle = relic; ctx.fill(); ctx.strokeStyle = item.color; ctx.lineWidth = nearby ? 4 : 2; ctx.stroke(); ctx.beginPath(); ctx.arc(node.x, node.y, 8, 0, Math.PI * 2); ctx.fillStyle = item.color; ctx.fill(); if (nearby) { ctx.fillStyle = '#0a1422ee'; ctx.fillRect(node.x - 82, node.y + 29, 164, 36); ctx.strokeStyle = `${item.color}dd`; ctx.lineWidth = 2; ctx.strokeRect(node.x - 82, node.y + 29, 164, 36); ctx.fillStyle = '#edf2e9'; ctx.font = 'bold 10px "DM Mono"'; ctx.textAlign = 'center'; ctx.fillText(item.name, node.x, node.y + 42); ctx.fillStyle = item.family === 'melee' ? '#ed725c' : item.family === 'ranged' ? '#edc968' : '#63d1c2'; ctx.font = '500 8px "DM Mono"'; ctx.fillText(item.family ? item.family.toUpperCase() : 'UPGRADE', node.x, node.y + 55); } });
